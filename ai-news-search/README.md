...
<img src="assets/logo.svg" width="110" alt="ai-news-search logo" />
...
python3 scripts/generate_report.py items.json report.md
python3 scripts/generate_report.py items.json report.html --html
```
...
./scripts/ensure_X_cookies.sh

# 验证 cookie 文件
python3 scripts/validate_X_cookies.py
```
...
| 文件 | 内容 |
| --- | --- |
| `references/ai-radar-sources.md` | 官方公告源、补充 RSS、本机 Folo 中观测到的真实 feed |
| `references/x-high-signal-accounts.txt` | 100+ 高信号 X 账号，按官方 → 核心人物 → 社区排序 |
| `references/github-hot-repos.md` | GitHub 雷达的关键词族、24h 排序规则、低质量识别 |

### Scripts（采集与报告工具）

| 脚本 | 作用 |
| --- | --- |
| `github_hot_ai_repos.py` | 关键词扫 GitHub 搜索 API，返回近 7 天热门 AI 仓库 |
| `github_trending_fast_risers.py` | 24h 活跃度优先的快速上升仓库，含 trending 页兜底 |
| `prepare_X_search.py` | 把关键词/账号转成 X 搜索链接，可一键打开 |
| `ensure_X_cookies.sh` | X 采集前置：自动校验、登录、导出 cookie |
| `generate_report.py` | 把归一化 JSON 渲染成 Markdown / HTML 报告 |
| `template.md` / `template.html` | 一页纸报告模板 |
...
A: 本项目由一段视频录像 + 本机 Folo 数据重建：结构、规则与文件清单按录像还原，脚本为按文档行为重新实现的功能等价版本；详见下方版本说明。
